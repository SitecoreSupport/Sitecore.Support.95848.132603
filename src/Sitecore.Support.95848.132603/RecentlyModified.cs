...
namespace Sitecore.Support.Buckets.Search.SearchDropdowns
{
    internal class RecentlyModified : ISearchDropDown
    {
        ...
        public RecentlyModified(IContentSearchConfigurationSettings settings)
        {
        ...
        {
            List<string> result;
            using (IProviderSearchContext providerSearchContext = ContentSearchManager.GetIndex((SitecoreIndexableItem)Context.ContentDatabase.GetItem(ItemIDs.RootID) as IIndexable).CreateSearchContext(SearchSecurityOptions.Default))
            {
                string format = FieldFormat.GetFieldFormat(providerSearchContext.Index, "__smallupdateddate", typeof(DateTime));
                IQueryable<SitecoreUISearchResultItem> queryable = (from i in providerSearchContext.GetQueryable<SitecoreUISearchResultItem>()
                                                                    where i["__smallupdateddate"].Between(DateTime.UtcNow.AddDays(-1.0).ToString(format, Context.ContentLanguage.CultureInfo), DateTime.UtcNow.AddDays(1.0).ToString(format, Context.ContentLanguage.CultureInfo), Inclusion.Both)
                                                                    select i into date
